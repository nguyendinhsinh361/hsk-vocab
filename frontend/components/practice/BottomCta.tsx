  ...
  return (
    <div className="absolute bottom-0 left-0 z-30 w-full px-4 pb-6 pt-3 flex justify-center bg-gradient-to-t from-white via-white to-transparent">
      <button
        ...
        onClick={onClick}
        className={cn(
          'w-full max-w-[21.4375rem] md:max-w-[84rem] h-12 md:h-14 rounded-full flex items-center justify-center gap-2',
          'font-sans font-semibold text-base transition-colors',
          disabled
            ? 'bg-neutral-200 text-neutral-400 cursor-not-allowed'
            : 'bg-primary border-b-4 border-[#008f85] text-white active:translate-y-[0.0625rem]',
        )}
      >
        {children}
        {withArrow && !disabled && (
