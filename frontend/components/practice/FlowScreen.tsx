...
/**
 * KHUNG luồng làm bài dùng chung cho LUYỆN TẬP (/practice/[root]) và
 * ÔN TẬP (/review): progress bar + timer + điều phối các màn theo step
 * (TEACH / PATTERN / QUIZ / MATCH) + màn tổng kết.
 * State machine nằm ở hooks/usePracticeFlow — component này chỉ render.
 */

import { useEffect, useState } from 'react';
import Link from 'next/link';
import type { PatternStep } from '@/lib/types';
import type { usePracticeFlow } from '@/hooks/usePracticeFlow';
...
import { MatchScreen } from '@/components/practice/MatchScreen';
...
export function FlowScreen({
  ...
  f: ReturnType<typeof usePracticeFlow>;
  backHref: string;
}) {
  ...
  return (
    <div className="relative min-h-[100dvh] w-full bg-white">
      <BrandBackdrop />
      {/* Mobile: full-width (đỡ trống 2 bên). Desktop: panel nội dung tự căn giữa (Figma web). */}
      <div className="relative z-10 mx-auto min-h-[100dvh] w-full bg-transparent">
        {f.phase !== 'summary' && (
          <div className="absolute left-0 top-0 z-20 w-full px-2 md:px-8 pt-4 md:pt-6 flex justify-center">
            <div className="flex items-center gap-3 md:gap-4 w-full md:max-w-[84rem]">
              <Link
                href={backHref}
                aria-label="Quay lại"
        ...
        {f.phase === 'error' && (
          <Centered>
            ...
            <button
              onClick={f.retry}
              className="mt-4 h-11 px-6 rounded-full bg-primary text-white font-semibold"
            >
              Thử lại
            </button>
          </Centered>
        ...
        {f.phase === 'step' && f.current?.kind === 'MATCH' && (
          <MatchScreen step={f.current} onNext={f.advance} />
        )}
        ...
        {f.phase === 'summary' && (
          <SummaryScreen
            correct={f.result?.correct ?? f.quizCorrect}
            total={f.result?.total ?? f.totalQuiz}
            xp={f.xpEarned}
            root={f.root}
